      ...
      <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-center gap-4">
        <div className="w-full flex-grow">
          ...
          <input
            ...
            disabled={isLoading}
            className="w-full h-10 px-3 bg-white dark:bg-slate-700 text-slate-900 dark:text-white border-slate-300 dark:border-slate-600 rounded-md shadow-sm focus:ring-sky-500 focus:border-sky-500 transition"
          />
        </div>
        <button
          type="submit"
          disabled={isLoading || !mood.trim()}
          className="w-full sm:w-auto h-10 flex items-center justify-center bg-sky-500 hover:bg-sky-600 disabled:bg-sky-500/50 dark:disabled:bg-sky-800 disabled:text-slate-100 dark:disabled:text-slate-400 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded-md transition-all duration-200 shadow-lg hover:shadow-sky-500/30"
        >
          {isLoading ? (
            <>
