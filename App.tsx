...
import { LoadingSpinnerIcon, HistoryIcon } from './components/icons';
...
const App: React.FC = () => {
  ...
  const TabButton: React.FC<{active: boolean, onClick: () => void, children: React.ReactNode}> = ({ active, onClick, children }) => (
    <button onClick={onClick} className={`px-4 py-2 text-sm font-medium rounded-md transition-colors ${active ? 'bg-sky-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}>
        {children}
    </button>
  );
      ...
      <div className="container mx-auto max-w-4xl">
        <header className="text-center mb-8">
            <div className="flex justify-center items-center gap-4 mb-4">
                ...
                <button onClick={() => setShowHistory(true)} className="relative p-2 rounded-full text-slate-300 hover:bg-slate-700 hover:text-white transition-colors" title="Open History">
                    <HistoryIcon className="h-7 w-7"/>
                    {savedProgressions.length > 0 && <span className="absolute top-0 right-0 block h-3 w-3 rounded-full bg-sky-500 ring-2 ring-slate-900"></span>}
                </button>
            </div>
        ...
        <main className="flex flex-col items-center gap-8">
          ...
          {error && (
            <div className="bg-red-500/20 border border-red-500 text-red-300 px-4 py-3 rounded-lg relative w-full max-w-2xl" role="alert">
          ...
          <div className="w-full">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center text-center p-12 bg-slate-800/50 rounded-xl">
                <LoadingSpinnerIcon className="h-12 w-12 text-sky-500" />
                <p className="mt-4 text-lg font-semibold text-slate-300">Crafting your progressions...</p>
              ...
              <div className="grid grid-cols-1 gap-6">
               ...
               !error && (
                <div className="text-center p-12 bg-slate-800/50 rounded-xl">
                    <h2 className="text-2xl font-bold text-white">Ready to Compose?</h2>
